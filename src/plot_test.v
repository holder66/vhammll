...
fn test_plot_hits() {
	...
	output = os.execute_or_panic('./temp rank -of -l 3 -b 2,5 vhammll/datasets/leukemia34test.tab')
	assert output.output.contains('Listening on')
	// weighted butterfly: normalised to percentages, symmetric x range [-100, 100]
	output = os.execute_or_panic('./temp rank -of -w -l 2 -b 2,4 vhammll/datasets/leukemia34test.tab')
	assert output.output.contains('Listening on')
}
