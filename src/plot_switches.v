...
fn plot_switches(result RankingResult) {
	...
	mut attrs := result.array_of_ranked_attributes.filter(
		it.attribute_type == 'C' && it.switches_array.len > 0)
	attrs.sort(array_min(a.switches_array) < array_min(b.switches_array))
